...
SELECT
	...
	(SELECT COUNT(*) FROM DIRECTOR_MAPPING) AS Director_Mapping_row_count,
...
SELECT Count(DISTINCT id) AS movies_US_INDIA, year
FROM movie
...
SELECT genre, ROUND(avg(duration),2) average_duration
FROM genre as g 
...
SELECT median_rating, COUNT(movie_id) AS number_of_movies
FROM movie AS m
INNER JOIN ratings AS r
ON m.id = r.movie_id
...
(
 ...
 (
	...
	ROW_NUMBER() OVER(ORDER BY COUNT(d.movie_id) DESC) AS dir_row_rank
	FROM
		names AS n 
